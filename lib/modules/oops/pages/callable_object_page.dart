...
import '../controllers/callable_object_controller.dart';

class CallableObjectPage extends GetView<CallableObjectController> {
  const CallableObjectPage({super.key});
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
            ...
            slivers: [
              if (!AS.deviceScreenType.isMobile)
                SliverAppBar(
                  ...
                  floating: false,
                  pinned: true,
                  flexibleSpace: PageHeader(headerName: SK.callableObject),
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      PreviousNextButton(
                        back: () => AppPages.router.go(AppPath.EXTENSION),
                        next: () => AppPages.router.go(AppPath.FACTORYCONSTRUCTOR),
                        isEnableBack: true,
                        isEnableNext: true,
                      ),
          ...
          GetBuilder<CallableObjectController>(builder: (_) => controller.showBackToTopButton ? Positioned(bottom: 20, right: 20, child: FloatingActionButton(backgroundColor: AppColors.primary700, onPressed: controller.scrollToTop, child: const Icon(Icons.arrow_upward, color: Colors.white))) : const SizedBox.shrink()),
        ],
      ),
    );
  }
