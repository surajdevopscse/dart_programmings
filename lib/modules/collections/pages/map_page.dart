    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
            ...
            slivers: [
              if (!AS.deviceScreenType.isMobile)
                SliverAppBar(
                  ...
                  floating: false,
                  pinned: true,
                  flexibleSpace: PageHeader(headerName: SK.mapInDart),
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const CommonHeight(height: 20),
                      BigText(text: controller.creatingMapTitle, style: AppStyle.globalBigTextStyle.copyWith(fontSize: 22, fontWeight: FontWeight.bold)),
                      const CommonHeight(height: 8),
                      BigText(text: controller.creatingMapDesc, style: AppStyle.globalSmallTextStyle.copyWith(fontSize: 16, fontWeight: FontWeight.normal)),
                      const CommonHeight(height: 8),
                      CodeWidget(code: controller.creatingMapCode),
                      const CommonHeight(height: 8),
                      CodeWidget(code: controller.creatingMapOutput),
                      const CommonHeight(height: 20),
                      BigText(text: controller.methodsTitle, style: AppStyle.globalBigTextStyle.copyWith(fontSize: 22, fontWeight: FontWeight.bold)),
                      const CommonHeight(height: 8),
                      BigText(text: controller.methodsDesc, style: AppStyle.globalSmallTextStyle.copyWith(fontSize: 16, fontWeight: FontWeight.normal)),
                      const CommonHeight(height: 8),
                      CodeWidget(code: controller.methodsCode),
                      const CommonHeight(height: 8),
                      CodeWidget(code: controller.methodsOutput),
                      const CommonHeight(height: 20),
                      ...
                      PreviousNextButton(
                        back: () => AppPages.router.go(AppPath.SETINDART),
                        next: () => AppPages.router.go(AppPath.WHERE),
                        isEnableBack: true,
                        isEnableNext: true,
                      ),
          ...
          GetBuilder<MapController>(builder: (_) => controller.showBackToTopButton ? Positioned(bottom: 20, right: 20, child: FloatingActionButton(backgroundColor: AppColors.primary700, onPressed: controller.scrollToTop, child: const Icon(Icons.arrow_upward, color: Colors.white))) : const SizedBox.shrink()),
        ],
      ),
    );
  }
