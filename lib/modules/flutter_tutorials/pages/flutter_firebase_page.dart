...
import '../controllers/flutter_firebase_controller.dart';

class FlutterFirebasePage extends GetView<FlutterFirebaseController> {
  const FlutterFirebasePage({super.key});
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
            ...
            slivers: [
              if (!AS.deviceScreenType.isMobile)
                SliverAppBar(
                  ...
                  floating: false,
                  pinned: true,
                  flexibleSpace: PageHeader(headerName: SK.flutterFirebase),
                  backgroundColor: C.primary50,
                  actions: [
                    BookmarkIconBtn(routePath: '${AppPath.FLUTTER_INTRO}/${AppPath.FLUTTER_FIREBASE}', pageTitle: SK.flutterFirebase),
                  ],
                ),
              SliverToBoxAdapter(
                child: Padding(
                  ...
                  child: ConstrainedBox(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Center(child: BigText(text: 'Firebase', style: AppStyle.globalBigTextStyle.copyWith(fontSize: 38, letterSpacing: 1.8, height: 2.5, fontWeight: FontWeight.normal))),
                        ...
                        const CommonHeight(height: 20),
                        BigText(text: 'Firestore CRUD', style: AppStyle.globalBigTextStyle.copyWith(fontSize: 22)),
                        const CommonHeight(height: 8),
                        CodeWidget(code: controller.firestoreCode),
                        const CommonHeight(height: 20),
                        BigText(text: 'Authentication', style: AppStyle.globalBigTextStyle.copyWith(fontSize: 22)),
                        const CommonHeight(height: 8),
                        CodeWidget(code: controller.authCode),
                        const CommonHeight(height: 20),
                        ...
                        TopicCompleteBtn(routePath: '${AppPath.FLUTTER_INTRO}/${AppPath.FLUTTER_FIREBASE}'),
                        const CommonHeight(height: 16),
                        PreviousNextButton(
                          back: () => AppPages.router.go('${AppPath.FLUTTER_INTRO}/${AppPath.FLUTTER_SQLITE}'),
                          next: () => AppPages.router.go('${AppPath.FLUTTER_INTRO}/${AppPath.FLUTTER_ANIMATIONS}'),
                        ),
          ...
          GetBuilder<FlutterFirebaseController>(
