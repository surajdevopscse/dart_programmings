...
import '../controllers/flutter_stateful_controller.dart';

class FlutterStatefulPage extends GetView<FlutterStatefulController> {
  const FlutterStatefulPage({super.key});
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
            ...
            slivers: [
              if (!AS.deviceScreenType.isMobile)
                SliverAppBar(
                  ...
                  floating: false,
                  pinned: true,
                  flexibleSpace: PageHeader(headerName: SK.flutterStateful),
                  backgroundColor: C.primary50,
                  actions: [
                    BookmarkIconBtn(routePath: '${AppPath.FLUTTER_INTRO}/${AppPath.FLUTTER_STATEFUL}', pageTitle: SK.flutterStateful),
                  ],
                ),
              SliverToBoxAdapter(
                child: Padding(
                  ...
                  child: ConstrainedBox(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Center(child: BigText(text: 'Stateful Widget', style: AppStyle.globalBigTextStyle.copyWith(fontSize: 38, letterSpacing: 1.8, height: 2.5, fontWeight: FontWeight.normal))),
                        const CommonHeight(height: 16),
                        BigText(text: controller.intro, style: AppStyle.globalSmallTextStyle.copyWith(fontSize: 18)),
                        const CommonHeight(height: 20),
                        BigText(text: 'Counter Example', style: AppStyle.globalBigTextStyle.copyWith(fontSize: 22)),
                        const CommonHeight(height: 8),
                        CodeWidget(code: controller.exampleCode),
                        const CommonHeight(height: 20),
                        BigText(text: 'Lifecycle Methods', style: AppStyle.globalBigTextStyle.copyWith(fontSize: 22)),
                        const CommonHeight(height: 8),
                        CodeWidget(code: controller.lifecycleCode),
                        const CommonHeight(height: 20),
                        BigText(text: 'Lifecycle Explained', style: AppStyle.globalBigTextStyle.copyWith(fontSize: 22)),
                        const CommonHeight(height: 8),
                        SmallText(text: controller.lifecycleDesc),
                        const CommonHeight(height: 20),
                        BigText(text: 'Tips', style: AppStyle.globalBigTextStyle.copyWith(fontSize: 22)),
                        const CommonHeight(height: 8),
                        ...controller.whenToUse.map((tip) => Padding(
                        ...
                        TopicCompleteBtn(routePath: '${AppPath.FLUTTER_INTRO}/${AppPath.FLUTTER_STATEFUL}'),
                        const CommonHeight(height: 16),
                        PreviousNextButton(
                          back: () => AppPages.router.go('${AppPath.FLUTTER_INTRO}/${AppPath.FLUTTER_STATELESS}'),
                          next: () => AppPages.router.go('${AppPath.FLUTTER_INTRO}/${AppPath.FLUTTER_LAYOUTS}'),
                        ),
          ...
          GetBuilder<FlutterStatefulController>(
