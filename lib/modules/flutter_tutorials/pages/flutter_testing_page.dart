...
import '../controllers/flutter_testing_controller.dart';

class FlutterTestingPage extends GetView<FlutterTestingController> {
  const FlutterTestingPage({super.key});
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
            ...
            slivers: [
              if (!AS.deviceScreenType.isMobile)
                SliverAppBar(
                  ...
                  floating: false,
                  pinned: true,
                  flexibleSpace: PageHeader(headerName: SK.flutterTesting),
                  backgroundColor: C.primary50,
                  actions: [
                    BookmarkIconBtn(routePath: '${AppPath.FLUTTER_INTRO}/${AppPath.FLUTTER_TESTING}', pageTitle: SK.flutterTesting),
                  ],
                ),
              SliverToBoxAdapter(
                child: Padding(
                  ...
                  child: ConstrainedBox(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Center(child: BigText(text: 'Testing', style: AppStyle.globalBigTextStyle.copyWith(fontSize: 38, letterSpacing: 1.8, height: 2.5, fontWeight: FontWeight.normal))),
                        const CommonHeight(height: 16),
                        BigText(text: controller.intro, style: AppStyle.globalSmallTextStyle.copyWith(fontSize: 18)),
                        const CommonHeight(height: 20),
                        BigText(text: 'Testing Pyramid', style: AppStyle.globalBigTextStyle.copyWith(fontSize: 22)),
                        const CommonHeight(height: 8),
                        SmallText(text: controller.testingTypesDesc),
                        const CommonHeight(height: 20),
                        BigText(text: 'Setup and Commands', style: AppStyle.globalBigTextStyle.copyWith(fontSize: 22)),
                        const CommonHeight(height: 8),
                        CodeWidget(code: controller.setupCode),
                        const CommonHeight(height: 20),
                        BigText(text: 'Unit Test Example', style: AppStyle.globalBigTextStyle.copyWith(fontSize: 22)),
                        const CommonHeight(height: 8),
                        CodeWidget(code: controller.unitTestCode),
                        const CommonHeight(height: 20),
                        ...
                        TopicCompleteBtn(routePath: '${AppPath.FLUTTER_INTRO}/${AppPath.FLUTTER_TESTING}'),
                        const CommonHeight(height: 16),
                        PreviousNextButton(
                          back: () => AppPages.router.go('${AppPath.FLUTTER_INTRO}/${AppPath.FLUTTER_HERO_ANIM}'),
                          next: () => AppPages.router.go('${AppPath.FLUTTER_INTRO}/${AppPath.FLUTTER_UNIT_TEST}'),
                        ),
          ...
          GetBuilder<FlutterTestingController>(
