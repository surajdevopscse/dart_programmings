...
import '../controllers/flutter_hero_anim_controller.dart';

class FlutterHeroAnimPage extends GetView<FlutterHeroAnimController> {
  const FlutterHeroAnimPage({super.key});
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
            ...
            slivers: [
              if (!AS.deviceScreenType.isMobile)
                SliverAppBar(
                  ...
                  floating: false,
                  pinned: true,
                  flexibleSpace: PageHeader(headerName: SK.flutterHeroAnim),
                  backgroundColor: C.primary50,
                  actions: [
                    BookmarkIconBtn(routePath: '${AppPath.FLUTTER_INTRO}/${AppPath.FLUTTER_HERO_ANIM}', pageTitle: SK.flutterHeroAnim),
                  ],
                ),
              SliverToBoxAdapter(
                child: Padding(
                  ...
                  child: ConstrainedBox(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Center(child: BigText(text: 'Hero Animation', style: AppStyle.globalBigTextStyle.copyWith(fontSize: 38, letterSpacing: 1.8, height: 2.5, fontWeight: FontWeight.normal))),
                        const CommonHeight(height: 16),
                        BigText(text: controller.intro, style: AppStyle.globalSmallTextStyle.copyWith(fontSize: 18)),
                        const CommonHeight(height: 20),
                        BigText(text: 'Hero Widget', style: AppStyle.globalBigTextStyle.copyWith(fontSize: 22)),
                        const CommonHeight(height: 8),
                        CodeWidget(code: controller.heroCode),
                        const CommonHeight(height: 20),
                        BigText(text: 'List to Detail Pattern', style: AppStyle.globalBigTextStyle.copyWith(fontSize: 22)),
                        const CommonHeight(height: 8),
                        CodeWidget(code: controller.listDetailExample),
                        const CommonHeight(height: 20),
                        ...
                        TopicCompleteBtn(routePath: '${AppPath.FLUTTER_INTRO}/${AppPath.FLUTTER_HERO_ANIM}'),
                        const CommonHeight(height: 16),
                        PreviousNextButton(
                          back: () => AppPages.router.go('${AppPath.FLUTTER_INTRO}/${AppPath.FLUTTER_EXPLICIT_ANIM}'),
                          next: () => AppPages.router.go('${AppPath.FLUTTER_INTRO}/${AppPath.FLUTTER_TESTING}'),
                        ),
          ...
          GetBuilder<FlutterHeroAnimController>(
